...
do_list_entries() {
    ...
    keepassxc-cli ls --no-password ${DATABASE_PATH} "${group_path}" --key-file=${KEYFILE_PATH} | while read line; do
        ...
        if [[ $line == */ ]]; then
            ...
            do_list_entries "${new_group_path%/}" # Remove trailing slash
        else
            ...
            echo keepassxc-cli show --no-password --quiet ${DATABASE_PATH} "${group_path}/${line}" --key-file=${KEYFILE_PATH} --attributes=Password \| xclip
